...
color_data_path = os.path.join(package_path, "data.json")

with open(color_data_path) as data_file:
...
def get(hex_code):
    ...
    for name, color in color_data.items():
        ...
        rgb_diff = pow(r - _r, 2) + pow(g - _g, 2) + pow(b - _b, 2)
        hsl_diff = pow(h - _h, 2) + pow(s - _s, 2) + pow(L - _L, 2)
