...
CASE_FUNC_MAP = {
    "dash": case_conversion.dashcase,
    "snake": case_conversion.snakecase,
    "camel": case_conversion.camelcase,
    "pascal": case_conversion.pascalcase,
    "screaming_snake": case_conversion.screaming_snakecase,
}
...
class Vanilla:
    ...
    def __init__(self, settings):
        ...
        use_tabs = settings.get("use_tabs", False)

        self.ruleset = RuleSet(selector=selector, use_tabs=use_tabs)
    ...
    def get_varname_hex_map(self, css):
        ...
        for rule_set in self.get_rulesets(css):
            ...
            varname_hex = [
                (n, hexutils.normalize(h))
                for n, h in varname_hex
    ...
    def convert_case(self, color_name):
        return self.name_case_func(text=color_name)
    ...
    def get_declarations(self, colorname_hex_map: dict):
        ...
        declarations = [
            create_declaration(n, colorname_hex_map[n]) for n in sorted_names
        ]
    ...
    def natural_sort(string):
        return [
            int(s) if s.isdigit() else s.lower() for s in re.split(r"([0-9]+)", string)
        ]
...
PREPROCESSOR_PREFIX_MAP = {"stylus": "$", "sass": "$", "scss": "$", "less": "@"}
